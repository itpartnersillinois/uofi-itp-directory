...
namespace uofi_itp_directory_data.Helpers {
    ...
    public class LookupHelper(DirectoryRepository directoryRepository) {
        ...
        public async Task<List<LookupThinObject>> GetAreas(string filter) => [
                .. (await _directoryRepository.ReadAsync(d => d.Areas.Where(a => a.Title.Contains(filter)).OrderBy(a => a.Title)
        ...
        public async Task<List<LookupThinObject>> GetOffices(string filter) => [
                .. (await _directoryRepository.ReadAsync(d => d.Offices.Include(o => o.Area).Where(o => o.Title.Contains(filter) || o.Area.Title.Contains(filter)).OrderBy(o => o.Title)
