...
namespace uofi_itp_directory_data.Security {
    ...
    public class PersonOptionHelper(DirectoryRepository directoryRepository) {
        ...
        public async Task<IEnumerable<AreaOfficeThinObject>> Areas(string? name) {
            ...
            var items = await _directoryRepository.ReadAsync(c => c.SecurityEntries.Where(p => p.NetId == name && p.IsActive).ToList());
        ...
        public async Task<bool> IsAreaAdmin(string? name, int areaId) => CheckParameters(name) &&
                            (await _directoryRepository.ReadAsync(c => c.SecurityEntries.Any(p => p.NetId == name && p.IsActive && (p.IsFullAdmin || p.AreaId == areaId))));
        ...
        public async Task<bool> IsOfficeAdmin(string? name, int officeId) {
            ...
            return await _directoryRepository.ReadAsync(c => c.SecurityEntries.Any(p => p.NetId == name && p.IsActive && (p.IsFullAdmin || p.AreaId == areaId || p.OfficeId == officeId)));
        }
        ...
        public async Task<IEnumerable<AreaOfficeThinObject>> Offices(string? name) {
            ...
            var items = await _directoryRepository.ReadAsync(c => c.SecurityEntries.Where(p => p.NetId == name && p.IsActive));
            if (items.Any(i => i.IsFullAdmin)) {
